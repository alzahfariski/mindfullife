...
class DassStressController extends GetxController {
  static DassStressController get instance => Get.find();

  RxList<DassModel> dassDataStress = <DassModel>[].obs;

  RxInt result = 0.obs;
  RxInt hal = 0.obs;
  RxString stressStatus = "Normal".obs;
  ...
  void onInit() {
    super.onInit();
    dassDataStress.assignAll(DassData.dassDataStress);
  }

  void nextTidak() {
    if (dassDataStress.length == hal.value + 1) {
      findStatus();
      result = result + 0;
      Get.to(() => const ResultCekKondisiPage());
    } else {
      result = result + 0;
      hal = hal + 1;
    }
  ...
  void findStatus() {
    ...
    if (result.value > 34) {
      stressStatus.value = "Sangat Parah";
